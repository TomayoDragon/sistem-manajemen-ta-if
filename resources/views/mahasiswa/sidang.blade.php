<x-mahasiswa-layout>
    ...
    <style>
        ...
        /* ... (style .status-message-box, .status-pending, .status-reject tetap sama) ... */
        .status-message-box {
            border-radius: 8px;
            padding: 40px;
            text-align: center;
            ...
            @if ($sidang)
                ...
                @elseif ($sidang->status == 'LULUS' || $sidang->status == 'TIDAK_LULUS')
                    ...
                    <div class="hasil-ba-box status-{{ strtolower($sidang->status) }}">
                        ...
                        <a href="{{ route('mahasiswa.sidang.berita-acara', $sidang->id) }}" target="_blank" style="
                        /* Style kustom untuk tombol download yang lebih menonjol */
                        ...
                        margin-top: 30px; /* Jarak yang lebih baik dari grid nilai */
